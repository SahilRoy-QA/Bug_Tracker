        ...
        <div className="flex items-center justify-between py-2 sm:py-2.5 gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 text-xs shrink-0">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white shadow-xs transition shrink-0"
            >
              <PlusCircle className="w-3.5 h-3.5 shrink-0" />
              <span className="hidden xs:inline">Log</span>
              <span className="hidden sm:inline">Defect</span>
            </button>

            {/* User Profile Dropdown & Direct Logout */}
            {onLogout && (
              <div className="flex items-center gap-1.5 pl-1.5 border-l border-slate-200 dark:border-slate-800">
                ...
                <div className="relative" ref={userMenuRef}>
                  <button 
                    ...
                    className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800/80 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-mono border border-slate-200 dark:border-slate-700/60 transition cursor-pointer"
                    title={`User Menu: ${currentUser}`}
                    aria-expanded={isUserMenuOpen}
                    aria-label="Toggle user account dropdown"
                  >
                    <User className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
                    <span className="font-semibold text-[11px] hidden sm:inline">{currentUser}</span>
                    <ChevronDown className={`w-3 h-3 text-slate-400 transition-transform duration-150 ${isUserMenuOpen ? 'rotate-180' : ''}`} />
                  </button>

                  {/* Dropdown Menu */}
                  {isUserMenuOpen && (
                    <div className="absolute right-0 top-full mt-2 w-56 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-2xl py-2 z-50 text-xs animate-in fade-in zoom-in-95 duration-100">
                ...
                <button
                  onClick={onLogout}
                  className="inline-flex items-center gap-1 px-2 sm:px-2.5 py-1.5 rounded-lg text-xs font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition border border-rose-200/80 dark:border-rose-900/60 shadow-xs cursor-pointer group"
                  title="Sign out of QA Dashboard"
                  aria-label="Sign out"
                >
